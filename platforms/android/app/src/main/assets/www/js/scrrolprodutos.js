...
$(document).ready(function(e) {
	...
	$.ajax({
			 ...
			 data:{act:"listarprodutosporum",cod:1,codcli:codc,idtabelapreco:codtptab},
			 dataType:"json",
			 beforeSend: function(){
				 $.mobile.loading( 'show', {
		...
		ListaUltComprasCliente(codc);
		
});	

function mostratabpreco(){

	var tabpreco = $("#tabpreco").val();
	...
	if(tabpreco == 1) {
		...
		$.ajax({
			 ...
			 data:{act:"buscatabpreco"},
			 dataType:"json",
			 success: function(data){
				
			 	var htm = '';
			 	var opt = '';
			 	htm +='<div class="ui-field-contain">'+
				    //'<label for="select-native-1">Selecione Uma tabela de preço:</label>'+
						'<select name="select-tabpreco" id="select-tabpreco" class="ui-btn ui-icon-carat-d ui-btn-icon-right ui-corner-all ui-shadow" style="width: 100%;">';
						htm+='<option value="" selected>Seleciona uma tabela de preço</option>';
						for (var i = 0; i < data.arr.length; i++) {
								var ret = data.arr[i];
								var sel = '';
								if(data.idtabpreco == ret.cod){
									sel = 'selected';	
								}
								htm+='<option value="'+ret.cod+'" '+sel+'>'+ret.desc+'</option>';
								opt+='<option value="'+ret.cod+'" '+sel+'>'+ret.desc+'</option>';
							}			
						        
				...
				        htm +='</select>'+
					...
					$('select[name="seltabpreco"]').html(opt);
					$( "select[name='seltabpreco']" ).selectmenu( "refresh", true );

				if(data.idtabpreco == ""){						
					dlog = $.dialog({
						type: 'orange',
						closeIcon:false,
					    title: 'Selecione um Tipo de tabela',
					    content: ''+htm+'',
					});				
				}
			},
...
$(document).on('change','select[name="select-tabpreco"]',function(){

		var idtabpreco = $(this).val();	
		$("#idtabelapreco").val(idtabpreco);
		$("select[name='seltabpreco'] option[value='"+idtabpreco+"']").attr("selected","selected");
		$( "select[name='seltabpreco']" ).selectmenu( "refresh", true );
...
$(document).on('change','select[name="seltabpreco"]',function(){
	var idtabpreco = $(this).val();

	if(parseFloat($(".s-card-count").html()) == 0){
		...
		$.ajax({
			 ...
			 data:{act:"trocatabelapreco",idtabpreco:idtabpreco},
			 dataType:"json",
			 success: function(data){
		...
		$.confirm({
			...
			content: 'Para mudar de Tabela você precisa excluir os item do carrinho!',
			type: 'orange',
			typeAnimated: true,
			buttons: {
				tryAgain: {
				...
				success: function(data){
					...
					if(data.length > 0){
						for (var i = 0; i < data.length; i++) {
							...
							var htmitemmaismenos = "";
							if(data[i].volume > 0){
								var countqtd = $("#volume"+codigo+"").val();	
							}else{
...
$(document).on('click','.deltalhe-produto',function(){
	...
	//alert($("a[data-id='"+$(this).attr('data-id')+"']").attr('data-url'));
	...
	$.ajax({
		...
		success: function(data){
			...
			//$(".quantidade").attr('id','quantidade'+data.detalhe[0].cod+'');
